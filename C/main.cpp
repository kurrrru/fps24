...
int main() {
    ...
    auto fact = factorial_mod(S + N, MOD);
    auto fact_inv = factorial_inv_mod(S + N, MOD);
    ...
    for (long long k = 0; k <= S; k++) {
        ...
        result += (combination_mod(N, k / (M + 1), MOD, fact, fact_inv) * (k / (M + 1) % 2 == 0 ? 1 : -1) + MOD) % MOD *
                    combination_mod(N + S - k - 1,  N - 1, MOD, fact, fact_inv) % MOD;
    }
